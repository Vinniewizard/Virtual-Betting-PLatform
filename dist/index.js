...
loadEnvFromFile(path_1.default.join(process.cwd(), '.env'));
...
const resolveAssetPath = (...segments) => {
    const candidates = [
        path_1.default.join(process.cwd(), ...segments),
        path_1.default.join(__dirname, ...segments),
        path_1.default.join(__dirname, '..', ...segments),
    ];
...
function sanitizeRiskSettings(next, current) {
...
function sanitizeAutoPlayConfig(userId, payload, current, risk) {
    const enabled = typeof (payload === null || payload === void 0 ? void 0 : payload.enabled) === 'boolean' ? payload.enabled : current.enabled;
    const parsedAmount = parseAmount(payload === null || payload === void 0 ? void 0 : payload.amount);
    const amount = parsedAmount === null ? current.amount : parsedAmount;
    const autoCashOut = typeof (payload === null || payload === void 0 ? void 0 : payload.autoCashOut) === 'boolean' ? payload.autoCashOut : current.autoCashOut;
    const parsedTarget = parseAmount(payload === null || payload === void 0 ? void 0 : payload.targetMultiplier);
...
const server = http_1.default.createServer((req, res) => {
    var _a;
    if ((_a = req.url) === null || _a === void 0 ? void 0 : _a.startsWith(ENGINE_IO_PATH)) {
        return;
    }
    router(req, res, (0, finalhandler_1.default)(req, res));
});
// Initialize Engine.IO for real-time communication
const engine = new engine_io_1.Server({
    cors: {
        origin: '*',
    },
});
...
engine.on('connection_error', (err) => {
    console.error('Engine.IO connection error:', {
        ...
        context: err === null || err === void 0 ? void 0 : err.context,
    });
});
// --- Session & DB Stores ---
...
const autoPlayCache = new Map();
const db = new database_1.DatabaseService();
// Initialize Game Engine
const aviatorGame = new game_1.AviatorGame({ curve: 'linear', multiplierStep: 0.01, tickIntervalMs: 100, maxMultiplier: 1000 });
const jetxGame = new game_1.AviatorGame({ curve: 'exp', expRate: 1.012, tickIntervalMs: 90, maxMultiplier: 500 });
const spacemanGame = new game_1.AviatorGame({ curve: 'exp', expRate: 1.014, tickIntervalMs: 85, maxMultiplier: 350 });
const crashGame = new game_1.AviatorGame({ curve: 'linear', multiplierStep: 0.012, tickIntervalMs: 95, maxMultiplier: 800 });
const balloonGame = new game_1.AviatorGame({ curve: 'exp', expRate: 1.009, tickIntervalMs: 110, maxMultiplier: 200 });
const diceGame = new game_1.DiceGame();
let riskSettings = sanitizeRiskSettings(db.getRiskSettings(DEFAULT_RISK_SETTINGS), DEFAULT_RISK_SETTINGS);
db.setRiskSettings(riskSettings);
const crashGameTypes = [
    types_1.GameType.AVIATOR,
    types_1.GameType.JETX,
    types_1.GameType.SPACEMAN,
    types_1.GameType.CRASH,
    types_1.GameType.BALLOON,
];
const crashGameMap = new Map([
    [types_1.GameType.AVIATOR, aviatorGame],
    [types_1.GameType.JETX, jetxGame],
    [types_1.GameType.SPACEMAN, spacemanGame],
    [types_1.GameType.CRASH, crashGame],
    [types_1.GameType.BALLOON, balloonGame],
]);
function isCrashGameType(gameType) {
    return crashGameMap.has(gameType);
}
function getCrashGame(gameType) {
    return crashGameMap.get(gameType) || null;
}
function setCrashGamesPaused(paused) {
...
function getDefaultAutoPlayConfig(userId) {
    return {
        userId,
        enabled: false,
        amount: 10,
...
function getAutoPlayConfig(userId) {
...
function persistAutoPlayConfig(config) {
    db.setAutoPlayConfig(config);
    autoPlayCache.set(config.userId, config);
    return config;
}
function broadcastRiskSettings() {
    broadcast('RISK_SETTINGS', riskSettings);
}
function applyRiskSettings(nextSettings, adminId) {
...
function isMaintenanceBlockedForUser(user) {
...
function enforceRuntimeAccess(socket, user, opts) {
    if (!user) {
    ...
    if (!(opts === null || opts === void 0 ? void 0 : opts.allowBanned) && user.status === 'banned') {
        sendError(socket, 'This account is banned.', 'ACCOUNT_BANNED');
        return false;
    }
    if (!(opts === null || opts === void 0 ? void 0 : opts.allowFrozen) && user.status === 'frozen') {
...
function sendSessionPayload(socket, user, token, type = 'LOGIN_SUCCESS') {
    const crashBets = {};
    crashGameTypes.forEach((gameType) => {
        const game = getCrashGame(gameType);
        if (!game) {
    ...
    const activeBet = crashBets[types_1.GameType.AVIATOR] || null;
...
function creditHouseOnLostBets(lostBets) {
...
function creditHouseForBetLoss(bet) {
    const houseAdmin = resolveHouseAdmin();
    if (!houseAdmin) {
        return;
    }
...
function debitHouseForBetWin(bet, payout) {
    const houseAdmin = resolveHouseAdmin();
    if (!houseAdmin) {
        return;
    }
...
function getBlackjackTotal(cards) {
    let total = 0;
    let aces = 0;
    cards.forEach((card) => {
        total += card.value;
...
function processReferralCommission(user, betAmount) {
    if (!user.referredBy)
        return;
...
function placeBetForUser(userId, amount, gameType = types_1.GameType.AVIATOR) {
    ...
    if (user)
        db.updateDailyLeaderboard(user.id, user.username, amount);
    return { ok: true, bet, newBalance };
}
function placeDiceBetForUser(userId, amount, condition, target) {
    const user = db.findUserById(userId);
    if (!user)
        return { ok: false, message: 'User not found.', code: 'USER_NOT_FOUND' };
    if (isMaintenanceBlockedForUser(user))
        return { ok: false, message: 'Maintenance mode.', code: 'MAINTENANCE_MODE' };
    if (user.balance < amount)
        return { ok: false, message: 'Insufficient funds.', code: 'INSUFFICIENT_FUNDS' };
    if (target < 2 || target > 98)
        return { ok: false, message: 'Target must be between 2 and 98.', code: 'INVALID_TARGET' };
    ...
    const bet = {
        ...
        userId: user.id,
        amount: Number(amount.toFixed(2)),
        gameType: types_1.GameType.DICE,
        gameId: crypto_1.default.randomUUID(), // Instant game, unique ID
        entryMultiplier: 1.0,
        exitMultiplier,
        payout,
        status,
...
function cashOutForUser(userId, gameType = types_1.GameType.AVIATOR, multiplier) {
    const user = db.findUserById(userId);
    if (!user) {
        return { ok: false, message: 'User not found.', code: 'USER_NOT_FOUND' };
    }
    ...
    const finalMultiplier = Number((multiplier !== null && multiplier !== void 0 ? multiplier : gameState.multiplier).toFixed(2));
...
function processAutoBetsForRound(state, gameType) {
    ...
    if (gameType !== types_1.GameType.AVIATOR) {
        return;
    }
    ...
    configs.forEach((config) => {
        ...
        const result = placeBetForUser(config.userId, config.amount, types_1.GameType.AVIATOR);
        if (result.ok) {
...
function processAutoCashOut(state, gameType) {
    ...
    activeBets.forEach((bet) => {
        ...
        const result = cashOutForUser(bet.userId, types_1.GameType.AVIATOR, state.multiplier);
        if (!result.ok) {
            return;
        }
...
function sendUserRiskSettings(socket) {
...
function handleRegister(socket, payload) {
    (async () => {
        try {
            ...
            const recoveryKey = crypto_1.default.randomBytes(16).toString('hex');
            const newUser = {
                id: crypto_1.default.randomUUID(),
                username,
...
function handleAuthResume(socket, payload) {
    const token = typeof (payload === null || payload === void 0 ? void 0 : payload.token) === 'string' ? payload.token : '';
    if (!token) {
...
async function handleResetPassword(socket, payload) {
    try {
...
function handleSetAutoPlay(socket, payload) {
    ...
    if (nextConfig.enabled && gameState.status === 'waiting' && !riskSettings.gamePaused && !riskSettings.maintenanceMode) {
        const result = placeBetForUser(user.id, nextConfig.amount, types_1.GameType.AVIATOR);
        if (result.ok) {
...
function handlePlaceBet(socket, payload) {
    ...
    const amount = parseAmount(payload === null || payload === void 0 ? void 0 : payload.amount);
    if (amount === null) {
        return sendError(socket, 'Invalid bet amount.', 'INVALID_AMOUNT');
    }
    const requestedType = payload === null || payload === void 0 ? void 0 : payload.gameType;
    const gameType = requestedType && isCrashGameType(requestedType) ? requestedType : types_1.GameType.AVIATOR;
...
function handlePlaceDiceBet(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const amount = parseAmount(payload === null || payload === void 0 ? void 0 : payload.amount);
    const condition = payload === null || payload === void 0 ? void 0 : payload.condition;
    const target = Number(payload === null || payload === void 0 ? void 0 : payload.target);
    if (amount === null || amount <= 0)
        return sendError(socket, 'Invalid amount.', 'INVALID_AMOUNT');
    if (condition !== 'over' && condition !== 'under')
        return sendError(socket, 'Invalid condition.', 'INVALID_INPUT');
    if (!Number.isFinite(target))
        return sendError(socket, 'Invalid target.', 'INVALID_INPUT');
...
function handleMinesStartGame(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const amount = parseAmount(payload === null || payload === void 0 ? void 0 : payload.amount);
    const mineCount = Number(payload === null || payload === void 0 ? void 0 : payload.mineCount);
    if (amount === null || amount <= 0)
        return sendError(socket, 'Invalid amount.', 'INVALID_AMOUNT');
    if (!Number.isInteger(mineCount) || mineCount < 1 || mineCount > 24)
        return sendError(socket, 'Mine count must be between 1 and 24.', 'INVALID_INPUT');
    if (user.balance < amount)
    ...
    const bet = {
        ...
        userId: user.id,
        amount: amount,
        gameType: types_1.GameType.MINES,
        gameId: crypto_1.default.randomUUID(), // This will be the MinesGame ID
        entryMultiplier: 1.0,
        status: 'active',
        timestamp: Date.now(),
    };
...
function handleMinesRevealTile(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const tileIndex = Number(payload === null || payload === void 0 ? void 0 : payload.tileIndex);
    ...
    if (game.revealedTiles.length === 0) {
        game.minesLayout = game_1.MinesGameLogic.generateMines(game.gridSize, game.mineCount, tileIndex);
    }
    if (game.minesLayout.includes(tileIndex)) {
        ...
        game.payoutMultiplier = game_1.MinesGameLogic.calculateMultiplier(game.revealedTiles.length, game.gridSize, game.mineCount);
        game.updatedAt = Date.now();
        db.updateMinesGame(game);
        const nextMultiplier = game_1.MinesGameLogic.calculateMultiplier(game.revealedTiles.length + 1, game.gridSize, game.mineCount);
        sendToSocket(socket, 'MINES_TILE_REVEALED', { minesGame: game, nextMultiplier });
    }
}
function handleMinesCashout(socket) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const game = db.getActiveMinesGameForUser(user.id);
...
function handlePlinkoDrop(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const amount = parseAmount(payload === null || payload === void 0 ? void 0 : payload.amount);
    const rows = Number(payload === null || payload === void 0 ? void 0 : payload.rows);
    const risk = payload === null || payload === void 0 ? void 0 : payload.risk;
    if (amount === null || amount <= 0)
        return sendError(socket, 'Invalid amount.', 'INVALID_AMOUNT');
    if (![8, 16].includes(rows))
        return sendError(socket, 'Invalid rows. Must be 8 or 16.', 'INVALID_INPUT');
    if (!['low', 'medium', 'high'].includes(risk))
        return sendError(socket, 'Invalid risk level.', 'INVALID_INPUT');
    if (user.balance < amount)
        return sendError(socket, 'Insufficient funds.', 'INSUFFICIENT_FUNDS');
    const newBalancePre = Number((user.balance - amount).toFixed(2));
    db.updateUserBalance(user.id, newBalancePre);
    const result = game_1.PlinkoGameLogic.playRound(rows, risk);
    ...
    const bet = {
        ...
        userId: user.id,
        amount: amount,
        gameType: types_1.GameType.PLINKO,
        gameId: crypto_1.default.randomUUID(), // Instant game, unique ID
        entryMultiplier: 1.0,
...
function handleHiloStartGame(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const amount = parseAmount(payload === null || payload === void 0 ? void 0 : payload.amount);
    ...
    const bet = {
        ...
        userId: user.id,
        amount: amount,
        gameType: types_1.GameType.HILO,
    ...
    db.createBet(bet);
    const now = Date.now();
    const deck = game_1.HiloGameLogic.createDeck();
...
function handleHiloBet(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const betDirection = payload === null || payload === void 0 ? void 0 : payload.bet;
    ...
    const { won } = game_1.HiloGameLogic.playRound(betDirection, previousCard, nextCard);
    if (won) {
        game.payoutMultiplier = game_1.HiloGameLogic.calculateMultiplier(game.history.length - 1);
        game.updatedAt = Date.now();
...
function handleHiloCashout(socket) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const game = db.getActiveHiloGameForUser(user.id);
...
function handlePlaceRouletteBet(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    ...
    const spinResult = game_1.RouletteGame.spinRoulette(betType, betValue);
    // Determine payout
    let payout = 0;
    let status = 'lost';
    ...
    const bet = {
        ...
        userId: user.id,
        amount: Number(amount.toFixed(2)),
        gameType: types_1.GameType.ROULETTE,
        gameId: crypto_1.default.randomUUID(),
        entryMultiplier: 1.0,
        exitMultiplier,
        payout,
...
function handleBlackjackPlay(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const amount = parseAmount(payload === null || payload === void 0 ? void 0 : payload.amount);
    ...
    const hand = { cards: playerCards, betAmount: amount, doubled: false, completed: false };
    const session = {
        userId: user.id,
        ...
        currentHand: 0,
        dealerCards,
        betId: crypto_1.default.randomUUID(),
    };
...
function finalizeBlackjackSession(socket, session) {
    ...
    const status = totalPayout > 0 ? 'cashed_out' : 'lost';
    const bet = {
        id: session.betId,
        userId: session.userId,
        amount: Number(totalBet.toFixed(2)),
        gameType: types_1.GameType.BLACKJACK,
        gameId: crypto_1.default.randomUUID(),
        entryMultiplier: 1.0,
        exitMultiplier,
    ...
    if (totalPayout > 0) {
        updateTournamentScores(bet);
        if (status === 'cashed_out')
            debitHouseForBetWin(bet, totalPayout);
    }
    else {
        creditHouseForBetLoss(bet);
    }
    ...
    if (user)
        db.updateDailyLeaderboard(user.id, user.username, totalBet);
    sendToSocket(socket, 'BLACKJACK_RESULT', {
        stage: 'final',
        isFinal: true,
        outcomes,
...
function handleBlackjackHit(socket) {
    const session = blackjackSessions.get(socket.id);
    if (!session)
        return sendError(socket, 'No active blackjack session.', 'NO_SESSION');
...
function handleBlackjackStand(socket) {
    const session = blackjackSessions.get(socket.id);
    if (!session)
        return sendError(socket, 'No active blackjack session.', 'NO_SESSION');
    const hand = session.hands[session.currentHand];
...
function handleBlackjackSplit(socket) {
    const session = blackjackSessions.get(socket.id);
    if (!session)
        return sendError(socket, 'No active blackjack session.', 'NO_SESSION');
    const hand = session.hands[session.currentHand];
    if (hand.cards.length !== 2)
        return sendError(socket, 'Can only split on initial two cards.', 'INVALID_SPLIT');
    const [c1, c2] = hand.cards;
    // simple rank compare (face value)
    const value = (c) => c.rank; // 'K', '10', etc.
    if (value(c1) !== value(c2))
        return sendError(socket, 'Cards must be same rank to split.', 'INVALID_SPLIT');
    // verify balance for additional bet
    const user = getUserBySocket(socket);
    if (!user)
        return sendError(socket, 'User not found.', 'INTERNAL');
    if (user.balance < hand.betAmount)
        return sendError(socket, 'Insufficient funds to split.', 'INSUFFICIENT_FUNDS');
    // deduct extra bet
    ...
    const newHand1 = { cards: [cardA, drawCard()], betAmount: hand.betAmount, doubled: false, completed: false };
    const newHand2 = { cards: [cardB, drawCard()], betAmount: hand.betAmount, doubled: false, completed: false };
...
function handleBlackjackDouble(socket) {
    const session = blackjackSessions.get(socket.id);
    if (!session)
        return sendError(socket, 'No active blackjack session.', 'NO_SESSION');
    const hand = session.hands[session.currentHand];
    if (hand.cards.length !== 2 || hand.doubled)
        return sendError(socket, 'Cannot double now.', 'INVALID_DOUBLE');
    const user = getUserBySocket(socket);
    if (!user)
        return sendError(socket, 'User not found.', 'INTERNAL');
    if (user.balance < hand.betAmount)
        return sendError(socket, 'Insufficient funds to double.', 'INSUFFICIENT_FUNDS');
    // deduct extra bet
    const newBal = Number((user.balance - hand.betAmount).toFixed(2));
    db.updateUserBalance(user.id, newBal);
    session.preBalance = newBal;
...
function handleBaccaratPlay(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const amount = parseAmount(payload === null || payload === void 0 ? void 0 : payload.amount);
    ...
    let betOn = 'player';
    if ((payload === null || payload === void 0 ? void 0 : payload.choice) === 'banker' || (payload === null || payload === void 0 ? void 0 : payload.choice) === 'tie') {
        betOn = payload.choice;
    }
    else if ((payload === null || payload === void 0 ? void 0 : payload.betOn) === 'banker' || (payload === null || payload === void 0 ? void 0 : payload.betOn) === 'tie') {
        betOn = payload.betOn;
    }
    let outcome = 'lose';
    ...
    const bet = {
        ...
        userId: user.id,
        amount: Number(amount.toFixed(2)),
        gameType: types_1.GameType.BACCARAT,
        gameId: crypto_1.default.randomUUID(),
        entryMultiplier: 1.0,
        exitMultiplier,
        payout,
...
function handleTeenPattiPlay(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const amount = parseAmount(payload === null || payload === void 0 ? void 0 : payload.amount);
    ...
    const bet = {
        ...
        userId: user.id,
        amount: Number(amount.toFixed(2)),
        gameType: types_1.GameType.TEEN_PATTI,
        gameId: crypto_1.default.randomUUID(),
        entryMultiplier: 1.0,
        exitMultiplier,
        payout,
...
function handleGridGamePlay(socket, payload, gameType) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
    const amount = parseAmount(payload === null || payload === void 0 ? void 0 : payload.amount);
    ...
    if (user.balance < amount)
        return sendError(socket, 'Insufficient funds.', 'INSUFFICIENT_FUNDS');
    const riskConfig = resolveGridRisk(payload === null || payload === void 0 ? void 0 : payload.risk);
    const newBalancePre = Number((user.balance - amount).toFixed(2));
    db.updateUserBalance(user.id, newBalancePre);
    ...
    const status = won ? 'cashed_out' : 'lost';
...
function handleCashOut(socket, payload) {
    var _a;
    const user = getUserBySocket(socket);
    ...
    console.log(`User ${user.username} cashed out at ${result.bet.exitMultiplier}x. Payout: ${(_a = result.bet.payout) === null || _a === void 0 ? void 0 : _a.toFixed(2)}`);
}
function handleDeposit(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user)) {
        return;
    }
...
function handleWithdraw(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user)) {
        return;
    }
...
function handleClaimDailyBonus(socket) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user)) {
        return;
    }
...
function handleTransferFunds(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user)) {
        return;
    }
...
function handleUpdateProfile(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user)) {
        return;
    }
...
async function handleChangePassword(socket, payload) {
    const user = getUserBySocket(socket);
    if (!enforceRuntimeAccess(socket, user))
        return;
...
function handleChatMessage(socket, payload) {
    ...
    const message = typeof (payload === null || payload === void 0 ? void 0 : payload.message) === 'string' ? payload.message.trim() : '';
    if (!message) {
        return;
    }
...
function handleAdminSetRisk(socket, payload) {
    const user = getUserBySocket(socket);
    if (!user) {
    ...
    const incoming = {
        minBet: typeof (payload === null || payload === void 0 ? void 0 : payload.minBet) === 'number' ? payload.minBet : undefined,
        maxBet: typeof (payload === null || payload === void 0 ? void 0 : payload.maxBet) === 'number' ? payload.maxBet : undefined,
        maintenanceMode: typeof (payload === null || payload === void 0 ? void 0 : payload.maintenanceMode) === 'boolean' ? payload.maintenanceMode : undefined,
        gamePaused: typeof (payload === null || payload === void 0 ? void 0 : payload.gamePaused) === 'boolean' ? payload.gamePaused : undefined,
    };
    applyRiskSettings(sanitizeRiskSettings(incoming, riskSettings), user.id);
    sendToSocket(socket, 'RISK_SETTINGS', riskSettings);
}
function handleAdminSendDirectMessage(socket, payload) {
    const user = getUserBySocket(socket);
    if (!user) {
        return sendError(socket, 'Authentication required.', 'AUTH_REQUIRED');
    }
...
function handleAdminForceCrash(socket, payload) {
    const user = getUserBySocket(socket);
    if (!user) {
        return sendError(socket, 'Authentication required.', 'AUTH_REQUIRED');
    }
...
router.get('/gamestate', (req, res) => {
    const requestUrl = new url_1.URL(req.url || '/', `http://${req.headers.host || 'localhost'}`);
    const gameTypeParam = requestUrl.searchParams.get('gameType');
    const gameType = gameTypeParam && isCrashGameType(gameTypeParam) ? gameTypeParam : types_1.GameType.AVIATOR;
...
router.get('/fairness/current', (req, res) => {
    const requestUrl = new url_1.URL(req.url || '/', `http://${req.headers.host || 'localhost'}`);
    const gameTypeParam = requestUrl.searchParams.get('gameType');
    const gameType = gameTypeParam && isCrashGameType(gameTypeParam) ? gameTypeParam : types_1.GameType.AVIATOR;
    const game = getCrashGame(gameType) || aviatorGame;
...
router.get('/risk-settings', (_req, res) => {
    sendJson(res, 200, riskSettings);
});
router.get('/admin/users', authenticateAdmin, (_req, res) => {
    sendJson(res, 200, db.getAllUsers());
});
router.get('/admin/house-summary', authenticateAdmin, (_req, res) => {
    const houseAdmin = resolveHouseAdmin();
    sendJson(res, 200, {
...
router.get('/admin/active-users', authenticateAdmin, (_req, res) => {
...
router.get('/admin/bets', authenticateAdmin, (req, res) => {
...
router.get('/admin/risk-settings', authenticateAdmin, (_req, res) => {
    sendJson(res, 200, riskSettings);
});
router.post('/admin/risk-settings', authenticateAdmin, async (req, res) => {
    try {
        const body = await getJsonBody(req);
        const incoming = {
            minBet: typeof (body === null || body === void 0 ? void 0 : body.minBet) === 'number' ? body.minBet : undefined,
            maxBet: typeof (body === null || body === void 0 ? void 0 : body.maxBet) === 'number' ? body.maxBet : undefined,
            maintenanceMode: typeof (body === null || body === void 0 ? void 0 : body.maintenanceMode) === 'boolean' ? body.maintenanceMode : undefined,
            gamePaused: typeof (body === null || body === void 0 ? void 0 : body.gamePaused) === 'boolean' ? body.gamePaused : undefined,
        };
...
router.post('/admin/balance', authenticateAdmin, async (req, res) => {
    try {
        const body = await getJsonBody(req);
...
router.post('/admin/user-status', authenticateAdmin, async (req, res) => {
    try {
        const body = await getJsonBody(req);
...
router.get('/admin/promocodes', authenticateAdmin, (_req, res) => {
    sendJson(res, 200, db.getAllPromoCodes());
});
router.post('/admin/promocodes', authenticateAdmin, async (req, res) => {
    try {
        const body = await getJsonBody(req);
...
router.post('/promocodes/redeem', authenticateToken, async (req, res) => {
    try {
        const body = await getJsonBody(req);
        const code = String((body === null || body === void 0 ? void 0 : body.code) || '').toUpperCase();
...
router.get('/leaderboard/daily', (_req, res) => {
...
router.get('/tournaments/:id/leaderboard', (req, res) => {
    ...
    if (!id)
        return sendJson(res, 400, { error: 'Tournament ID required' });
...
router.post('/admin/tournaments', authenticateAdmin, async (req, res) => {
    try {
        const body = await getJsonBody(req);
...
router.post('/admin/tournaments/score', authenticateAdmin, async (req, res) => {
    try {
        const body = await getJsonBody(req);
...
router.get('/transactions/me', authenticateToken, (req, res) => {
    const userId = req.user.id;
    const { page, limit, offset } = getPagination(req);
...
router.get('/bets/me', authenticateToken, (req, res) => {
    const userId = req.user.id;
    const { page, limit, offset } = getPagination(req);
...
router.get('/autoplay/me', authenticateToken, (req, res) => {
    const userId = req.user.id;
    sendJson(res, 200, getAutoPlayConfig(userId));
});
router.post('/autoplay/me', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const user = db.findUserById(userId);
        if (!user) {
...
// Game event handling
function registerCrashGameEvents(gameType, game) {
        ...
        lostBets.forEach((lostBet) => {
            ...
            if (socketId && engine.clients[socketId]) {
                ...
                sendToSocket(engine.clients[socketId], 'BET_LOST', {
                    bet: lostBet,
                    betId: lostBet.id,
                    newBalance: user === null || user === void 0 ? void 0 : user.balance,
                });
            }
        });
    });
        ...
        try {
            ...
            switch (message.type) {
                ...
                case 'BLACKJACK_DOUBLE':
                    ...
                    break;
                case 'PLAY_TOWER':
                    handleGridGamePlay(socket, message.payload, types_1.GameType.TOWER);
                    break;
                case 'PLAY_DRAGON_TOWER':
                    handleGridGamePlay(socket, message.payload, types_1.GameType.DRAGON_TOWER);
                    break;
                case 'PLAY_TREASURE_HUNT':
                    handleGridGamePlay(socket, message.payload, types_1.GameType.TREASURE_HUNT);
                    break;
